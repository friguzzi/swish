...
draw_traj(N):-
    sample_trajectory(N,Ob,St),
  Ob=[[O1x,O1y]|Rest],
    term_string(c(O1x),CX1),
    term_string(c(O1y),CY1),
    term_string(c(0),CZ1),
          <- library("animation"),
<-library("plot3D"),
 ...
 saveGIF({
    scatter3D(",
    S2=", 
xlim=c(-10,10), ylim=c(-10,10), zlim=c(-1,3),
            bty=\"b2\",
                    col = \"blue\", 
ticktype = \"detailed\", lwd = 4)
     scatter3D(",
    build_string(Rest,[O1x],[O1y],[0],S),
    atomics_to_string([S1,CX1,",",CY1,",",CZ1,S2|S],Comm),
 r_execute([],Comm,_),
    r_download.

build_string([[HX,HY]],X0,Y0,Z0,[CXS,",",CYS,",",CZS,S3]):-!,
    append(X0,[HX],X),
    append(Y0,[HY],Y),
    CX=..[c|X],
    CY=..[c|Y],
  CZ=..[c,0|Z0],
    ...
    term_string(CZ,CZS),
        S3=",   type = \"l\", 
            col = \"blue\", shade = 0.2, contour = TRUE,
            bty=\"b2\",col.panel=FALSE,
    xlim=c(-10,10), ylim=c(-10,10), zlim=c(-1,3),type = \"l\",
ticktype = \"detailed\", lwd = 4)   

}, interval = 0.2, ani.width = 550, ani.height = 550)".

build_string([[HX,HY]|Rest],X0,Y0,Z0,[CXS,",",CYS,",",CZS,S3|S]):-
    append(X0,[HX],X),
    append(Y0,[HY],Y),
    CX=..[c|X],
    CY=..[c|Y],
